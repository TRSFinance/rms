 ...
 */
public class TransInfoWeiboACSPCUtils  extends TransInfoUtils {
	/**
	 * 自身默认实例
	 */
	private static TransInfoWeiboACSPCUtils instance;

	/**
	 * 构造函数
	 */
	public TransInfoWeiboACSPCUtils() {
	 ...
	 * @return
	 */
	public static TransInfoWeiboACSPCUtils getDefaultInstance() {
		if (instance == null) {
			instance = new TransInfoWeiboACSPCUtils();
		}
		return instance;
	}
	...
	public CTransInfo generateCTransInfo(String _sTransModelDefine, PDMEditTrans _pageDataModel, MailConfigBean _mailConfig) throws STATransException {
		...
		_sourceInfo.setSearchCondition(_sSourceSearchExpression.replace("baotou_gongan.alarmcenterconfig", _sSourceDBService + "." + _sSourceDBTable));
		...
		_paramList[1] = _sSourceDBHost; // 数据处理参数列表
		_paramList[2] = _sSourceDBPort;
		_paramList[3] = _sSourceDBUser;
		_paramList[4] = _sSourceDBPassword;
		_paramList[5] = _sSourceDBService;
