 ...
 */
public class TransInfoOMHotWordUtils extends TransInfoUtils {
	/**
	 * 自身默认实例
	 */
	private static TransInfoOMHotWordUtils instance;

	/**
	 * 构造函数
	 */
	public TransInfoOMHotWordUtils() {
	 ...
	 * @return
	 */
	public static TransInfoOMHotWordUtils getDefaultInstance() {
		if (instance == null) {
			instance = new TransInfoOMHotWordUtils();
		}
		return instance;
	}
	...
	private CTransInfo generateCTransInfo(String _sTransModelDefine, PDMEditTrans _pageDataModel, int _iDateInter) throws STATransException {
		...
		_processInfoList = _cTransInfo.getChildCTransInfos()[0].getProcessInfos();
		...
		_paramList[12] = _sSourceDBHost; // 数据处理参数列表
		_paramList[13] = _sSourceDBPort;
		_paramList[14] = _sSourceDBUser;
		_paramList[15] = _sSourceDBPassword;
		_paramList[16] = _sSourceDBTable;
