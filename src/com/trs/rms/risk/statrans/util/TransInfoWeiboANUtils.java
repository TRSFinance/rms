 ...
 *
 */
public class TransInfoWeiboANUtils extends TransInfoUtils {
	/**
	 * 自身默认实例
	 */
	private static TransInfoWeiboANUtils instance;

	/**
	 * 构造函数
	 */
	public TransInfoWeiboANUtils() {
	 ...
	 * @return
	 */
	public static TransInfoWeiboANUtils getDefaultInstance() {
		if (instance == null) {
			instance = new TransInfoWeiboANUtils();
		}
		return instance;
	}
	...
	public CTransInfo generateCTransInfo(String _sTransModelDefine, PDMEditTrans _pageDataModel, MailConfigBean _mailConfig) throws STATransException {
		...
		_sourceInfo.setSearchCondition(_sSourceSearchExpression.replace("om3_baotou.weiboalarmanalysis", _sSourceDBService + "." + _sSourceDBTable));
	...
	public static void main(String args[]) {
		try {
			TransInfoWeiboANUtils _instance = TransInfoWeiboANUtils.getDefaultInstance();

			String _sRetValue = _instance.parseSourceTable("select id,alarmId,incrementInHour from om3_baotou.weiboalarmanalysis where alarmId>0");
			System.out.println(_sRetValue);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
